...
| 2026-03-24 | 新增TODO：优化ClickHouse去重策略（ReplacingMergeTree最终一致性问题） | - |
...
**问题描述**：
- ReplacingMergeTree是最终一致性，非即时一致性
- INSERT时不立即去重，后台合并时机不可控
- 查询时不带FINAL会返回重复数据

**待办内容**：
1. 调研最佳去重方案
2. 实现写入前去重逻辑（sync_service层）
3. 配置定时OPTIMIZE TABLE任务
4. 或查询统一使用FINAL修饰符

**相关文档**：docs/stock_sync.md
