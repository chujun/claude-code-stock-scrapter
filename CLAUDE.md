...
# 初始化财务风险表（首次运行前必须执行）
clickhouse-client --multiquery < storage/migrations/003_stock_financial_risk.sql
...
| stock_financial_risk | 财务风险数据 | ReplacingMergeTree |
...
同花顺财务风险数据爬虫模块（ths_client.py），通过Playwright爬取同花顺网站的财务风险评估数据。
...
# 创建客户端
ths_client = THSClient(headless=True, rate_limit_interval=2.0)
repo = ClickHouseRepository()

# 创建服务并同步
service = FinancialRiskService(repo=repo, playwright_client=ths_client)
